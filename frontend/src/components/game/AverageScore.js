...
const AverageScore = () => {
  ...
  const [averageScore, setAverageScore] = useState(() => {
    const savedAverageScore = localStorage.getItem('averageScore');
  ...
  useEffect(() => {
    ...
    socket.on('sendPlayerCount', (playerCount) => {
      ...
      localStorage.setItem('playerCount', playerCount.playerCount);
    });
  ...
  useEffect(() => {
    if (scores.length > 0 && playerCount > 0) {
      ...
      localStorage.setItem('averageScore', avgScore.toFixed(2));
    }
  }, [scores, playerCount]);
